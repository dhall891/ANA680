...
# Trees don’t need feature scaling and can capture non‑linear splits, but they can overfit—so we’ll set a few sensible constraints.
# 
...
# 2) model (simple, regularized a bit)
dt_clf = DecisionTreeClassifier(
    criterion="gini",
    max_depth=None,          # try values like 3–6 later in tuning if needed
    min_samples_split=2,
    min_samples_leaf=1,
    random_state=42
)
...
# 4) log results
evaluate_and_log("Decision Tree", y_test, y_pred_dt)
...
# probabilities (trees support predict_proba)
...
# calibration
prob_true, prob_pred = calibration_curve(y_test, y_proba_dt, n_bins=10, strategy="uniform")
...
ax_cal.plot(prob_pred, prob_true, marker="o", linewidth=1.5, label="Decision Tree")
...
fig.suptitle(
    f"Decision Tree Diagnostic Panel\n"
...
# **Key Metrics:**  
# - **Accuracy:** 95.9%  
# - **Precision:** 90.8% — Slightly more false positives than some earlier models.  
# - **Recall (Sensitivity):** 98.3% — Nearly all malignant cases correctly identified.  
# - **Specificity:** 94.6% — Strong benign classification.  
# - **F1 Score:** 94.4% — Balanced performance between precision and recall.  
# - **ROC AUC:** 0.965 — Strong class separability.  
# - **PR AUC:** 0.898 — Good, though slightly lower than other top models.
# 
# **Confusion Matrix:**  
...
# 
# **Interpretation:**  
# The decision tree achieved high recall (98.3%), meaning it correctly identified nearly all malignant cases. However, its precision (90.8%) and specificity (94.6%) were slightly lower than some earlier models, showing a small trade-off with more false positives. The ROC and PR curves are strong but not as smooth as linear or kernel SVMs, and the calibration curve shows a consistent under-prediction at higher probability ranges. This performance suggests the tree is effective but might benefit from ensemble techniques like Random Forest to reduce variance while maintaining recall.
# 
