...
# 
# Random Forest averages many decision trees to reduce variance (overfitting) while keeping non‑linear power.  
# Settings: `RandomForestClassifier(n_estimators=10, random_state=42, class_weight=None)`.  
...
# 4) log
evaluate_and_log("Random Forest (n=10)", y_test, y_pred_rf)


# In[8]:


# === Metrics & curves (imports if needed) ===
...
# calibration
prob_true, prob_pred = calibration_curve(y_test, y_proba_rf, n_bins=10, strategy="uniform")
...
ax_cal.plot(prob_pred, prob_true, marker="o", linewidth=1.5, label="Random Forest")
...
fig.suptitle(
    f"Random Forest (n=10) Diagnostic Panel\n"
...
# ### Random Forest (n=10) — Model Performance Summary
# 
# **Key Metrics:**  
# - **Accuracy:** 95.9% — High overall classification performance.  
# - **Precision:** 92.1% — Strong ability to correctly identify malignant cases with minimal false positives.  
# - **Recall (Sensitivity):** 96.7% — Most malignant cases detected, slightly lower than the highest-recall models.  
# - **Specificity:** 95.5% — Robust benign classification.  
# - **F1 Score:** 94.3% — Balanced between precision and recall.  
# - **ROC AUC:** 0.975 — Excellent separability between classes.  
# - **PR AUC:** 0.914 — Strong performance in precision–recall trade-offs.  
# 
# **Confusion Matrix:**  
...
# 
# **Interpretation:**  
# The Random Forest model delivers strong and balanced performance, with high precision (92.1%) and recall (96.7%). Its ROC AUC of 0.975 and PR AUC of 0.914 show excellent classification ability across thresholds. While recall is slightly lower than models like Kernel SVM or XGBoost, the trade-off is a well-controlled false positive rate. The calibration curve is relatively close to perfect calibration, though with minor deviations in the mid-probability range. This makes Random Forest a solid choice when both sensitivity and specificity are priorities.
# 
